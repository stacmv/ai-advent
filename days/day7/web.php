...
function getHistoryFile(): string
{
    ...
    return $storageDir . '/history_day7.json';
}
...
function handleRecordStart($env)
{
    ...
    $recordingFile = $recordDir . DIRECTORY_SEPARATOR . 'day7_' . $timestamp . '.mp4';
...
{
    try {
        $result = shell_exec('php ' . escapeshellarg(__DIR__ . '/../../tools/upload_latest.php') . ' 7 2>&1');
...
<head>
    ...
    <title>Day 7: Persistent History — Web UI</title>
    <style>
...
<body>
    <header>
        <div class="header-title">
            === Day 7: <span>Persistent Conversation History</span> ===
        </div>
    ...
    <form id="form" onsubmit="return false;">
        <input id="input" type="text" placeholder="Type your message… (history is persisted across sessions)" autofocus autocomplete="off">
    ...
    <script>
        ...
        async function sendMessage(messageText, historyFile) {
            ...
            try {
                ...
                if (historyFile) body.historyFile = historyFile;
                ...
                if (data.error) {
                    ...
                    addMsg('agent', data.response, 'History: ' + data.messageCount + ' messages');
                }
            } catch (e) {
                thinking.remove();
        ...
        async function runDemo() {
            ...
            try {
                ...
                for (let i = 0; i < data.cases.length; i++) {
                    ...
                    // Each demo case gets its own isolated history file
                    const sessionFile = null; // uses default file, cleared per session
                    addMsg('demo', '=== Session ' + (i + 1) + (c.name ? ': ' + c.name : '') + ' ===');
